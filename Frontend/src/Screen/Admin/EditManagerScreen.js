...
import { Register, UpdateUser, GetManagers } from "../../../redux/actions";
import { apiConfig } from '../../../redux/config'
import Loader from '../../Components/Loader';
import Header from '../../Components/Header'
class EditManagerScreen extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            name: '',
    ...
    componentDidUpdate = (previousProps, previousState) => {
        if (this.props.route.params.manager) {
            if (previousState.name !== this.props.route.params.manager.name) {
                var manager = this.props.route.params.manager;
                this.setState(
                    {
                        name: manager.name,
                        birth: manager.birth,
    ...
    validate = () => {
        ...
        if (this.state.oldPassword != '' && this.state.oldPassword != this.props.route.params.manager.password) {
            this.setState({ errPassword: true });
            return false;
        }
        if (this.props.route.params.manager && this.state.oldPassword == '' && (this.state.newPassword != '' || this.state.newPasswordConfirm != '')) {
            this.setState({ errPassword: true });
            return false;
        }
        if (this.state.oldPassword != '' && this.state.oldPassword == this.props.route.params.manager.password || !this.props.route.params.manager) {
        ...
        if (this.validate() == true) {
            ...
            if (this.props.route.params.manager) {
                this.props.updateUser(
                    this.props.route.params.manager._id,
                    this.createFormData(this.state.photo, {
                        name: this.state.name,
                        birth: this.state.birth,
                        phone: this.state.phone,
                        address: this.state.address,
                        newPassword: this.state.oldPassword ? this.state.newPassword : this.props.route.params.manager.password,
                        email: this.state.email,
                        thumbnail: this.state.photo ? timeVal + '-' + this.state.photo.fileName : '',
                        role: Roles.MANAGER
                    }, timeVal), false, () => {
                        // if (successcb) successcb();
                        console.log("Update manager data sucessfully")
                ...
                this.props.register(
                    this.createFormData(this.state.photo, {
                        ...
                        // if (successcb) successcb();
                        console.log("Add manager data sucessfully")
                        this.props.getManagers(
                            () => {
                                this.setState({ loading: false });
                                this.props.navigation.goBack();
    ...
    render() {
        return (
            <Block>
                ...
                <Header nologo title={this.props.route.params.manager ? "Edit Manager" : "Add Manager"} navigation={this.props.navigation} />
                <Block center style={styles.background}>
                    ...
                    >
                        <Block style={{ borderWidth: 1, borderRadius: nowTheme.SIZES.RADIUS }}>
                            <Image
                                source={this.state.photo ? { uri: this.state.photo.uri } : (this.props.route.params.manager && this.props.route.params.manager.thumbnail ?
                                    { uri: `${apiConfig.baseUrl}image/${this.props.route.params.manager.thumbnail}` } :
                                    Images.Avatar)}
                                style={styles.Avatar}
                            />
                        </Block>
                ...
                <SafeAreaView>
                    ...
                    >
                        ...
                        {this.props.route.params.manager &&
                            ...
                            />}
                        {this.props.route.params.manager && this.state.errPassword && <Text p color='red'>password is not correct</Text>}
                        <Input
