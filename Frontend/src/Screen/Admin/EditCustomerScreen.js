...
import { Register, UpdateUser, GetCustomers } from "../../../redux/actions";
...
class EditCustomerScreen extends React.Component {
    ...
    componentDidUpdate = (previousProps, previousState) => {
        if (this.props.route.params.customer) {
            if (previousState.name !== this.props.route.params.customer.name) {
                var customer = this.props.route.params.customer;
                this.setState(
                    {
                        name: customer.name,
    ...
    validate = () => {
        ...
        if (this.state.oldPassword != '' && this.state.oldPassword != this.props.route.params.customer.password) {
            this.setState({ errPassword: true });
            return false;
        }
        if (this.props.route.params.customer && this.state.oldPassword == '' && (this.state.newPassword != '' || this.state.newPasswordConfirm != '')) {
            this.setState({ errPassword: true });
            return false;
        }
        if (this.state.oldPassword != '' && this.state.oldPassword == this.props.route.params.customer.password || !this.props.route.params.customer) {
        ...
        if (this.validate() == true) {
            ...
            if (this.props.route.params.customer) {
                this.props.updateUser(
                    this.props.route.params.customer._id,
                    this.createFormData(this.state.photo, {
                        name: this.state.name,
                        birth: this.state.birth,
                        phone: this.state.phone,
                        address: this.state.address,
                        newPassword: this.state.oldPassword ? this.state.newPassword : this.props.route.params.customer.password,
                        email: this.state.email,
                        thumbnail: this.state.photo ? timeVal + '-' + this.state.photo.fileName : '',
                        role: Roles.CUSTOMER
                    }, timeVal), false, () => {
                        // if (successcb) successcb();
                        console.log("Update customer data sucessfully")
                ...
                this.props.register(
                    this.createFormData(this.state.photo, {
                        ...
                        // if (successcb) successcb();
                        console.log("Add customer data sucessfully")
                        this.props.getCustomers(
                            () => {
                                this.setState({ loading: false });
                                this.props.navigation.goBack();
                ...
                <SafeAreaView>
                    ...
                    >
                        ...
                        {this.props.route.params.customer &&
                            ...
                            />}
                        {this.props.route.params.customer && this.state.errPassword && <Text p color='red'>password is not correct</Text>}
                        <Input
